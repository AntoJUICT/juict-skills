...
terugkijk-venster dat naar boven open is: vanaf `periodEnd` minus
`checkLookbackMonths` maanden (default 12, instelbaar in `config.json`) tot
heden/toekomst, zonder bovengrens op `periodEnd`. Dat is bewust: een
afgerond, nog-niet-gepost ticket kan een `dateWorked` NA de billing-maand
hebben (bijvoorbeeld een onsite-bezoek van na de maandafsluiting, terwijl er
pas later wordt gepost), dus een bovengrens op `periodEnd` zou zulke pending
items ten onrechte missen. Zo blijft dit een volledig vangnet, zowel voor
oudere als voor recentere afgeronde, nog-niet-geposte onsite-tickets of
urennorm-overschrijdingen. De Remote Support-AI-digest ("Remote Support te
AI-checken") blijft wel strikt op de billing-maand staan: die uren worden
apart teruggefilterd naar `periodStart`/`periodEnd` voordat de digest wordt
opgebouwd, zodat de leeslijst voor Claude niet groeit met alles uit het
bredere venster.
