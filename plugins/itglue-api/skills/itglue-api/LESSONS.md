...
**Vergelijk padsegmenten zonder punt-suffix, en toets ook de `include`-parameter.** Twee gaten in diezelfde grens, gevonden bij de eindreview van 2026-07-31. Een vergelijking op het hele segment ziet `passwords.json` als een andere resource dan `passwords`, terwijl een server die `.json` als formaat leest daar bij dezelfde resource uitkomt; sindsdien knippen beide controles het punt-deel van elk segment af (`passwords.json`, `passwords.` en `/passwords.json/12345` gaan er dus uit). En de grens keek alleen naar het pad, dus `organizations/7?include=passwords` liep er langs terwijl een include de gerelateerde items in het `included`-deel van de body zet die `get` ruw print. `get` weigert nu ook een `include` die de passwords-resource noemt, komma-gescheiden en genest meegerekend (`include=passwords,configurations`, `include=organization.passwords`) en ongeacht casing. Andere queryparameters blijven erbuiten: een filterwaarde met het woord erin verandert niet welke resource terugkomt. Dit is beleid en geen uitspraak over wat IT Glue met zulke paden of parameters doet.
...
**De password-deeplinkvorm is gecorroboreerd, niet gemeten.** `https://juict.eu.itglue.com/<org-id>/passwords/<id>` komt niet alleen uit onze eigen code: het JUICT-project `huntress-bulk-onboarding` gebruikt exact deze vorm om het organisatie-id uit een portal-URL te halen (`extractOrgIdFromPasswordUrl()` in `src/itglue.ts`, met echte 16-cijferige IT Glue-id's in `test/itglue.test.ts` regel 172), en die URL komt daar uit de `--password-url`-vlag waar iemand hem uit de portal plakt. Gebruik in een werkend project dus, maar nog geen eigen portal-observatie: dat een door `password-link` gegenereerde link ook echt op het bedoelde item uitkomt heeft niemand van ons nagekeken. De documentenvariant `/<org-id>/docs/<id>` heeft die onderbouwing niet, die vonden we in dat project niet terug. Beide staan in de tabel met openstaande punten in REFERENCE.md; ze zijn afgevinkt zodra iemand één zo'n link in de portal heeft geopend.
